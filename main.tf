...
resource "aws_apigatewayv2_route" "ispar" {
  api_id = aws_apigatewayv2_api.lambda.id
...
resource "aws_lambda_permission" "api_gw_helloworld" {
...
resource "aws_lambda_permission" "api_gw_ispar" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
